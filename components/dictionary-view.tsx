...
export function DictionaryView({ onBack, refreshKey }: { onBack: () => void; refreshKey?: number }) {
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {entries.length === 0 ? (
          <p className="text-xs leading-relaxed text-muted-foreground">
            No words saved yet. Highlight any text in a conversation to save it here.
          </p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <span
                      className={cn("truncate text-sm", mastered && "text-green-500")}
                    >
                      {entry.word}
                    </span>
                    {mastered && (
                      <span className="shrink-0 text-[10px] text-green-500">mastered</span>
                    )}
                  </div>
                  <Button
                    size="icon-sm"
